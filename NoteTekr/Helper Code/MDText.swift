...
struct RegexMarkdown: Equatable {
    ...
    static func url(for string: String) -> String {
        let matcher = try! NSRegularExpression(pattern: #"((http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%.\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%\+.~#?&//=]*))"#)
...
enum BaseMarkdownRules: String, CaseIterable, MarkdownRule {
    ...
    var regex: RegexMarkdown {
        switch self {
        case .header:
            return .init(matchIn: #"(\*#)|(#\*)(.+?)(\*#)|(#\*)"#, matchOut: "$3", strategy: self.header(_:))
        case .link:
            return .init(matchIn: #"\[([^\[]+)\]\(([^\)]+)\)"#, matchOut: "$1", strategy: self.link(_:))
        case .bold:
            return .init(matchIn: #"(\*{1})(.+?)(\*{1})"#, matchOut: "$2", strategy: self.bold(_:))
        case .hyperlink:
            return .init(matchIn: "<((?i)https?://(?:www\\.)?\\S+(?:/|\\b))>", matchOut: "$1", strategy: self.link(_:))
        case .emphasis:
            return .init(matchIn: #"(#{1})(.+?)(#{1})"#, matchOut: "$2", strategy: self.emphasis(_:))
        case .none:
